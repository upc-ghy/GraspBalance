...
class GraspNetDataset(Dataset):
    def __init__(self, root, valid_obj_idxs, grasp_labels, camera='kinect', split='train', num_points=20000,
        ...
        if split == 'train':
            self.sceneIds = list(range(26,28))
    ...
    def project_cad_to_camera_pcd(self, index, camera_pose, align_mat, scene_points):
        ...
        for i in range(len(model_list)):
            ...
            # model = o3d.geometry.voxel_down_sample(model_list[i], 0.005)
            pcd_combined += model
        ...
        # dists = o3d.geometry.compute_point_cloud_distance(pcd_combined, scene_w_noise)
        dists = np.asarray(dists)
        ind = np.where(dists < 0.008)[0]
        # pcd_combined_crop = pcd_combined.select_by_index(ind)
        pcd_combined_crop = pcd_combined.select_down_sample(ind)
        # pcd_combined_crop = o3d.geometry.select_by_index(pcd_combined, ind)
